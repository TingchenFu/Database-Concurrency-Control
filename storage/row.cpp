...
#if ALGO==MVCC
	manager=(Row_mvcc*) alloc_memory.align_alloc(sizeof(Row_mvcc));
#endif
#if ALGO==OCC
	manager = (Row_occ *) alloc_memory.align_alloc(sizeof(Row_occ));
#endif
...
RC RowData::get_row(access_t type, TxnMgr * txn, RowData *& row) {
	...
	rc = this->manager->access(txn,type);
	row = txn->cur_row;
	goto end;
#endif
#if ALGO == OCCTEMPLATE
...
*/
	...
	assert (row != NULL);
	if (rc == Abort) {
		manager->abort(type,txn,this);
	} else {
		manager->commit(type,txn,this,row);
	}
