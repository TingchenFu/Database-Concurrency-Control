...
void TxnMgr::set_start_timestamp(ts_t start_timestamp){
  txn->start_timestamp=start_timestamp;
} 
ts_t TxnMgr::get_start_timestamp(){
...
RC TxnMgr::validate() {
  ...
  return RCOK;
#endif
  if (ALGO != OCCTEMPLATE && ALGO!=OCC && ALGO!=MVCC)  {
      return RCOK;
  }
  RC rc = RCOK;
  uint64_t begintime = acquire_ts();
...
#if ALGO==MVCC
  if (rc==RCOK){
    rc=mvcc_man.validate(this);
  }
#endif
  if(rc == RCOK && ALGO==OCCTEMPLATE) {
